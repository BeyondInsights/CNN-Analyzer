...
export default function ReportConfigModal() {
  ...
  const currentReportType = reportSettings.reportType;
  const currentOutputType = reportSettings.outputType;
  ...
  const handleSubmit = () => {
    setReportSettings({ reportType: selectedReportType, outputType: selectedOutputType });
    closeReportConfigModal();
  };
          ...
          <div className="space-y-2">
            ...
            <RadioGroup value={selectedReportType} onValueChange={(value) => setSelectedReportType(value as ReportType)}>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="tiered-bundles" id="rt-tiered" />
                <Label htmlFor="rt-tiered">Tiered Bundles</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="independent-products" id="rt-independent" />
                <Label htmlFor="rt-independent">Independent Products</Label>
              </div>
            </RadioGroup>
          </div>
          <div className="space-y-2">
            <Label className="font-semibold text-base">Output Type:</Label>
            <RadioGroup value={selectedOutputType} onValueChange={(value) => setSelectedOutputType(value as OutputType)}>
              ...
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="count" id="ot-population" />
