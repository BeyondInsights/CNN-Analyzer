...
interface ProfileDisplayProps {
  selectedProductsData: ProductSetupConfig[];
}
            ...
            {selectedProductsData.map(product => (
              <Card key={product.id} className="bg-muted/20">
                ...
                <CardContent className="p-3 text-xs space-y-1">
                  <p><strong>Monthly Rate:</strong> ${product.monthlyRate?.toFixed(2) || '0.00'}</p>
                  ...
                  {product.readerFeatures && product.readerFeatures.length > 0 && <p><strong>Reader:</strong> {product.readerFeatures.join(', ')}</p>}
                  {product.streamingFeatures && product.streamingFeatures.length > 0 && <p><strong>Streaming:</strong> {product.streamingFeatures.join(', ')}</p>}
                  {product.verticals && product.verticals.length > 0 && <p><strong>Verticals:</strong> {product.verticals.join(', ')}</p>}
                </CardContent>
              </Card>
            ))}
          </div>
