...
export async function runPriceSensitivityAnalysis(
  ...
  for (let i = 0; i < steps; i++) {
    ...
    const adoptionRate = sim ? sim.overallShare[0] : 0;
    const priceVariation = ((price - baseRate) / baseRate) * 100;

    results.push({
      priceVariation: priceVariation,
      testPrice: Number(price.toFixed(2)),
      adoptionRate: Number(Math.max(0, Math.min(100, adoptionRate)).toFixed(1)),
      adoptionImpact: `${priceVariation > 0 ? '+' : ''}${priceVariation.toFixed(1)}%`,
      sharePercentage: Number(Math.max(0, Math.min(100, adoptionRate)).toFixed(1)),
      estimatedRevenue: Math.round(Math.max(0, Math.min(100, adoptionRate)) / 100 * 105624640 * price * 12),
      elasticityUsed: -1.2 // Default elasticity
    });
  }
