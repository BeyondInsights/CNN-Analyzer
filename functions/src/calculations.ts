...
import { PRODUCT_PRICE_RANGES } from './constants';
...
function calculateUtility(
  ...
  Object.entries(readerFeatures).forEach(([f, val]) => {
    if (product.readerFeatures?.includes(f)) u += (val as number);
  });
  ...
  Object.entries(streamingFeatures).forEach(([f, val]) => {
    if (product.streamingFeatures?.includes(f)) u += (val as number);
  });
...
export function applyOverallMarketFactors(baseRate: number, factors: MarketFactors): number {
  const safeFactors = {
...
export function applyPriceSensitivity(
  respondentProductProbabilities: Map<string, Map<string, number>>,
  ...
  for (const [, probs] of respondentProductProbabilities) { // Removed _respondentId
    for (const product of activeProducts) {
      ...
      const productRange = PRODUCT_PRICE_RANGES[product.product as keyof typeof PRODUCT_PRICE_RANGES];
      if (!productRange) {
