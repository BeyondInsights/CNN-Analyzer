...
exports.runSimulation = functions.https.onCall(async (data) => {
    ...
    try {
        ...
        let respondentsRaw; // Expect an object with any structure initially
        let respondents;
        try {
            ...
            if (respondentsRaw && typeof respondentsRaw === 'object' && !Array.isArray(respondentsRaw)) {
                respondents = Object.entries(respondentsRaw).map(([id, rawParamData]) => {
                    ...
                    const { respondentId, // This might be redundant if 'id' is used, but keep if present in data
                    weight, cbc_version, drn, // Try to get drn
                    base, price, verticals, verticalCount, subscription, features, featureCounts, gender, // Include demographic fields if they exist at this level
                    ageGroup, ...other // Capture any other fields that might exist
                     } = rawParamData;
                    // Construct the RespondentWithParams object
                    const respondent = {
                        respondentId: respondentId || id,
                        weight: typeof weight === 'number' ? weight : 1,
                        cbc_version: typeof cbc_version === 'number' ? cbc_version : undefined,
                        drn: typeof drn === 'number' ? Math.max(0.1, Math.min(0.99, drn)) : 0.5,
                        individualParams: {
                            base: base || {},
                    ...
                    return respondent;
                }).filter(r => r !== null); // Filter out any nulls from bad data
