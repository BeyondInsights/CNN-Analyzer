...
admin.initializeApp();
exports.runSimulation = functions.https.onCall(async (data) => {
    // Authentication check (optional, can be re-enabled later)
    // if (!context.auth) {
    //   throw new functions.https.HttpsError('unauthenticated', 'The function must be called while authenticated.');
    // }
    const { products, // This should be ProductSetupConfig[]
    marketFactors, simulationOptions,
    ...
    try {
        ...
        if (!utilsFileLocationData || typeof utilsFileLocationData.fileName !== 'string') {
            console.error('"fileName" field missing or not a string in "SimulationData/utilitiesFileLocation".', utilsFileLocationData);
        ...
        try {
            ...
            throw new functions.https.HttpsError('internal', `Error processing utilities data file from Storage: ${error.message || 'Unknown error'}`);
        }
        ...
        if (!respDataFileLocationData || typeof respDataFileLocationData.fileName !== 'string') {
            console.error('"fileName" field missing or not a string in "SimulationData/respondentDataFileLocation".', respDataFileLocationData);
        ...
        try {
            ...
            respondents = JSON.parse(respFileContents.toString('utf8'));
            console.log(`Successfully downloaded and parsed respondent data. Number of respondents: ${respondents ? respondents.length : 'N/A'}`);
        }
        catch (error) {
            console.error(`Failed to download or parse respondent data file "${respondentDataFileName}" from Storage. Error:`, error);
            throw new functions.https.HttpsError('internal', `Error processing respondent data file from Storage: ${error.message || 'Unknown error'}`);
        }
        if (!Array.isArray(respondents)) {
            console.error(`Parsed respondent data from Storage file "${respondentDataFileName}" is NOT an array. Actual type:`, typeof respondents);
        ...
        // --- Run Simulation ---
        console.log(`Calling computeTakeRates with ${products.length} products, ` +
        ...
        const simulationResults = (0, calculations_1.computeTakeRates)(respondents, // Corrected: This should be the respondent data
        products, // Corrected: This is the product configuration
        simulationOptions, marketFactors
        ...
        console.error('Original error details for unexpected error:', error.message, error.stack);
        throw new functions.https.HttpsError('internal', `An unexpected error occurred during simulation. Check function logs. Message: ${error.message || 'No error message available'}`);
    }
});
